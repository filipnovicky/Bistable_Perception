function [sc, post, co] = switcher_BP(MDP,N,T,ze,nze)
...
th = 0.5+1e-16; % Probability threshold for counting a switch
post = zeros(T-1,nze);
co = 0;
if N==1
    s = zeros(1,2);
    for f1 = 1:2
        for t = 2:T
            ...
            u = MDP.u(2,t-1);
            if f1 == 1 && y > th && z < 1 - th && u == 3
                sc1 = 1;
                sc2 = 0;
                post(t-1,ze) = post(t-1,ze) + y; % save posterior beliefs
                co = co + 1;
            elseif f1 == 2 && y > th && z < 1 - th && u == 2
                sc2 = 1;
                sc1 = 0;
